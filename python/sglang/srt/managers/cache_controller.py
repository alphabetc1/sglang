...
class HiCacheController:
    ...
    def _stop_storage_threads(self):
        ...
        if alive:
            # Keep stop_event set to prevent any further progress and surface an error.
            logger.error(
    ...
    def detach_storage_backend(self):
        ...
        """
        # Idempotent / best-effort cleanup: even if `enable_storage` is already False,
        ...
        if self.enable_storage:
            ...
            # If storage was already marked disabled but threads exist, attempt to stop them.
            if (
                hasattr(self, "prefetch_thread")
                or hasattr(self, "backup_thread")
                or hasattr(self, "prefetch_io_aux_thread")
            ):
                try:
                    self._stop_storage_threads()
                except Exception:
                    # Do not raise here: caller can retry detach.
                    logger.exception("Best-effort stop storage threads failed.")
