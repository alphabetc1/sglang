...
from sglang.srt.models.llama import LlamaDecoderLayer as BaseLlamaDecoderLayer
...
class LlamaModel(nn.Module):
    def __init__(
        ...
        self.hidden_state_size = self.hidden_size_in * (
            3 if self.use_aux_hidden_state else 1
        )
        self.fc = nn.Linear(
        ...
        self.mask_hidden = nn.Parameter(torch.zeros(self.hidden_state_size))
...
class LlamaForCausalLMPeagle(LlamaForCausalLM):
    def __init__(
        ...
        self.ptd_token_id = getattr(config, "ptd_token_id", getattr(config, "pard_token", None))
        if self.ptd_token_id is None:
            raise ValueError(
