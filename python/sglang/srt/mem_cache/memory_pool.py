...
class HybridReqToTokenPool(ReqToTokenPool):
    ...
    def _init_mamba_pool(
        ...
        self.mamba_allocator = MambaTokenToKVPoolAllocator(
            size=mamba_size,
            device=device,
            kvcache=self.mamba_pool,
        )
    ...
    def alloc(self, reqs: List["Req"]) -> Optional[List[int]]:
        ...
        for req in reqs:
            if req.mamba_pool_idx is not None:  # for radix cache / continuing chunked
                ...
                assert (
                    mid is not None
                ), f"Not enough space for mamba cache, try to increase --mamba-full-memory-ratio or --max-mamba-cache-size. {mid=}, {self.mamba_allocator.size=}, {self.mamba_allocator.available_size()=}, {len(reqs)=}"
