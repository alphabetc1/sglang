...
DEFAULT_ADAPTIVE_EMA_ALPHA = 1.0
DEFAULT_ADAPTIVE_UPDATE_INTERVAL = 1
DEFAULT_ADAPTIVE_WARMUP_BATCHES = 1
DEFAULT_ADAPTIVE_DOWN_HYSTERESIS = 0.0
DEFAULT_ADAPTIVE_UP_HYSTERESIS = -0.25


def build_default_adaptive_candidate_steps(max_steps: int) -> List[int]:
    """Build sparse adaptive anchors for topk=1 EAGLE speculative decoding."""
    ...
    while step < max_steps:
        ...
        step *= 2
    candidate_steps.append(max_steps)
...
class AdaptiveSpeculativeParams:
    ...
    def create_for_eagle_topk1(
        ...
        candidate_steps = build_default_adaptive_candidate_steps(max_steps)
        return cls(
            initial_steps=initial_steps,
            max_steps=max_steps,
            min_steps=candidate_steps[0],
            ema_alpha=DEFAULT_ADAPTIVE_EMA_ALPHA,
            update_interval=DEFAULT_ADAPTIVE_UPDATE_INTERVAL,
            warmup_batches=DEFAULT_ADAPTIVE_WARMUP_BATCHES,
            down_hysteresis=DEFAULT_ADAPTIVE_DOWN_HYSTERESIS,
            up_hysteresis=DEFAULT_ADAPTIVE_UP_HYSTERESIS,
    ...
    def __init__(
        ...
        assert min_steps >= 1, "min_steps must be >= 1"
        assert max_steps >= min_steps, "max_steps must be >= min_steps"
        assert 0 < ema_alpha <= 1, "ema_alpha must be in (0, 1]"
        assert update_interval >= 1, "update_interval must be >= 1"
        assert warmup_batches >= 0, "warmup_batches must be >= 0"
        assert down_hysteresis >= 0, "down_hysteresis must be >= 0"
        assert up_hysteresis > -0.5, "up_hysteresis must be > -0.5"
