...
void chain_speculative_sampling_target_only(
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  cudaError_t status = sampling::ChainSpeculativeSamplingTargetOnly<float, int32_t, int64_t>(
      static_cast<int32_t*>(predicts.data_ptr()),
      static_cast<int32_t*>(accept_index.data_ptr()),
      static_cast<int32_t*>(accept_token_num.data_ptr()),
      static_cast<int64_t*>(candidates.data_ptr()),
  ...
  TORCH_CHECK(
      status == cudaSuccess,
      "ChainSpeculativeSamplingTargetOnly failed with error code " + std::string(cudaGetErrorString(status)));
}
