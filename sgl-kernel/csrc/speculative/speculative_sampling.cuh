...
template <
    ...
    typename IdType,
    typename IdType2>
__global__ void ChainSpeculativeSamplingTargetOnly(
    IdType* predicts,          // mutable
    IdType* accept_index,      // mutable
    IdType* accept_token_num,  // mutable
    IdType2* candidates,
    IdType2* retrive_index,
    IdType2* retrive_next_token,
    IdType2* retrive_next_sibling,  // ignored (assumed no siblings when topk==1)
    DType* uniform_samples,
    DType* uniform_samples_for_final_sampling,
    DType* target_probs,
    uint32_t batch_size,
  ...
  // In topk=1 (chain) case, there is at most one rejected draft token before we stop.
  IdType2 rejected_token_id = -1;
  ...
  for (uint32_t j = 1; j < num_speculative_tokens; ++j) {
    ...
    if (coin <= prob_acc / threshold_acc || target_prob_single >= threshold_single) {
      ...
      // reject and stop (no siblings in topk==1 case)
  ...
  // For topk==1 we sample from q, but if we rejected a draft token at current state,
  // we mask it out (equivalent to old behavior where draft_probs[token]=target_probs[token]).
  DType sum_q(0);
...
#pragma unroll
    for (uint32_t j = 0; j < VEC_SIZE; ++j) {
      ...
      DType v = q_vec[j];
      if (has_rejected && idx < d && idx == static_cast<uint32_t>(rejected_token_id)) {
        v = DType(0);
      }
      q_masked[j] = v;
    }
...
cudaError_t ChainSpeculativeSamplingTargetOnly(
  ...
  DISPATCH_ALIGNED_VEC_SIZE(
      vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
        auto kernel = ChainSpeculativeSamplingTargetOnly<
            BLOCK_THREADS,
            SCAN_ALGO,
            REDUCE_ALGO,
            VEC_SIZE,
