...
This can reduce GPU memory usage, improve inference speed, and help to avoid OOM, but may impact model performance, thus set a proper value based on your specific use case. The `image` keys are applied to processors that expose a HuggingFace `image_processor` (for example, Qwen-VL). The `video`/`audio` keys are currently supported by `qwen_vl` only. Please refer to [qwen_vl processor](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/multimodal/processors/qwen_vl.py) for understanding the meaning of each parameter.
