...
- **`--hicache-storage-backend-extra-config HICACHE_STORAGE_BACKEND_EXTRA_CONFIG`**: the extra config can be either
  ...
  - For `file` backend only: `capacity_gb` (float, default 0) and `ttl_seconds` (float, default 0). When both are 0, capacity/TTL eviction is disabled. The eviction policy is LRU.
